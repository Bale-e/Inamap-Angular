...
let BabylonSceneService = (() => {
    ...
    let _classThis;
    var BabylonSceneService = class {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            BabylonSceneService = _classThis = _classDescriptor.value;
        ...
        initScene(canvasContainer, onMeshClicked, onMarkersUpdated) {
            this.dispose();
        ...
        setupCamera(canvas) {
            if (!this.scene)
                return;
            this.camera = new BABYLON.ArcRotateCamera('camera1', Math.PI / 4, Math.PI / 3, 25, new BABYLON.Vector3(0, 0, 0), this.scene);
        ...
        async loadModel(modelName, building = 'A') {
            if (!this.scene)
                return;
            const buildingId = typeof building === 'boolean' ? (building ? 'B' : 'A') : building;
            ...
            try {
                const result = await BABYLON.SceneLoader.ImportMeshAsync('', encodeURI(rootUrl), encodeURI(modelName), this.scene);
        ...
        updateScreenMarkers() {
            if (!this.scene || !this.camera || !this.engine || !this.onMarkersUpdatedCb)
                return;
            const markers = [];
            ...
            const projectPointOnGround = (localX, localZ, heightOffset = 0.05) => {
                ...
                const screenCoords = BABYLON.Vector3.Project(adjustedWorldPos, BABYLON.Matrix.Identity(), transformMatrix, viewport);
                const isVisible = screenCoords.z > 0 &&
                    screenCoords.z < 1 &&
                    screenCoords.x > 0 &&
        ...
        drawAnimatedRoute(points) {
            ...
            const leveledPoints = points.map((p, i) => i === points.length - 1
        ...
        focusOnMesh(meshName, targetRadius = 6, durationMs = 700, flipAngle = false) {
        ...
        getMeshWorldPosition(locName, cuerpoId) {
            if (!this.scene)
                return null;
            const findMeshCI = (name) => {
                if (!name || !this.scene)
                    return null;
    ...
    return BabylonSceneService = _classThis;
})();
export { BabylonSceneService };
