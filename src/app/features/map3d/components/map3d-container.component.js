...
let Map3dContainerComponent = (() => {
    ...
    let _classThis;
    let _renderCanvasContainer_decorators;
    let _renderCanvasContainer_initializers = [];
    let _renderCanvasContainer_extraInitializers = [];
    let _floorSelectorComponent_decorators;
    let _floorSelectorComponent_initializers = [];
    let _floorSelectorComponent_extraInitializers = [];
    var Map3dContainerComponent = class {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(null) : void 0;
            _renderCanvasContainer_decorators = [ViewChild('renderCanvas', { static: false })];
            _floorSelectorComponent_decorators = [ViewChild(FloorSelectorComponent, { static: false })];
            __esDecorate(null, null, _renderCanvasContainer_decorators, { kind: "field", name: "renderCanvasContainer", static: false, private: false, access: { has: obj => "renderCanvasContainer" in obj, get: obj => obj.renderCanvasContainer, set: (obj, value) => { obj.renderCanvasContainer = value; } }, metadata: _metadata }, _renderCanvasContainer_initializers, _renderCanvasContainer_extraInitializers);
            __esDecorate(null, null, _floorSelectorComponent_decorators, { kind: "field", name: "floorSelectorComponent", static: false, private: false, access: { has: obj => "floorSelectorComponent" in obj, get: obj => obj.floorSelectorComponent, set: (obj, value) => { obj.floorSelectorComponent = value; } }, metadata: _metadata }, _floorSelectorComponent_initializers, _floorSelectorComponent_extraInitializers);
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            Map3dContainerComponent = _classThis = _classDescriptor.value;
            if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
            __runInitializers(_classThis, _classExtraInitializers);
        }
        renderCanvasContainer = __runInitializers(this, _renderCanvasContainer_initializers, void 0);
        floorSelectorComponent = (__runInitializers(this, _renderCanvasContainer_extraInitializers), __runInitializers(this, _floorSelectorComponent_initializers, void 0));
        viewMode = (__runInitializers(this, _floorSelectorComponent_extraInitializers), '3d');
        currentBuilding = 'A';
        currentFloorModel = 'Edifico A - Piso 1.obj';
        destinations = [];
        // Alias esperado por los tests
        get destinationOptions() { return this.destinations; }
        set destinationOptions(v) { this.destinations = v; }
        ...
        cameraBeforeRenderObserver = null;
        modelRoot = null;
        renderLoopFn = () => { };
        onDocumentClick = (_) => { };
        ...
        mapNavService;
        cd;
        firstFloorModel = 'Edifico A - Piso 1.obj';
        secondFloorModel = 'Edificio A - piso 2.obj';
        thirdFloorModel = 'Edificio A - piso 3.obj';
        ...
        static createForTest(firebaseService, _ngZone, cd) {
            const fakeBabylon = {
                initScene: () => { },
                loadModel: () => { },
            ...
            const fakeMapNav = {
                currentFloor$: { subscribe: (_) => ({ unsubscribe: () => { } }) },
                currentBuilding$: { subscribe: (_) => ({ unsubscribe: () => { } }) },
                destinations$: { subscribe: (_) => ({ unsubscribe: () => { } }) },
                loadDestinations: () => { },
                setFloor: (_) => { },
                calculateRoute: async (_) => null,
                getLocationInfoByMeshNameAsync: async (_) => null
            };
            const comp = new Map3dContainerComponent(fakeBabylon, fakeMapNav, cd);
            comp.firebaseService = firebaseService;
            return comp;
        }
        ...
        async loadLocationOptions() {
            if (!this.firebaseService || typeof this.firebaseService.getLocacionesDeTodosLosEdificios !== 'function')
                return;
        ...
        getLocationSearchValues(location) {
            const values = [];
            if (!location)
                return values;
            const pushOnce = (v) => { if (v != null) {
                const s = String(v);
        ...
        extractCoordinateFromLocation(location) {
            if (!location)
                return null;
        ...
        calculateOrthoSizeForFocus(size) {
        ...
        setViewportHoverState(state) {
            this.viewportHovered = !!state;
        }
        ngAfterViewInit() {
            if (this.renderCanvasContainer?.nativeElement) {
                this.babylonSceneService.initScene(this.renderCanvasContainer.nativeElement, (meshName, pickResult) => this.onMeshPicked(meshName, pickResult), (markers) => this.onMarkersUpdated(markers));
            }
            this.subscriptions.add(this.mapNavService.currentBuilding$.subscribe(building => {
                this.currentBuilding = building;
                this.cd.detectChanges();
            }));
            this.subscriptions.add(this.mapNavService.currentFloor$.subscribe(floorModel => {
            ...
            this.subscriptions.add(this.mapNavService.destinations$.subscribe(dests => {
        ...
        async onMeshPicked(meshName, pickResult) {
            try {
        ...
        animateCameraFocus(targetVector, targetOrthoSize, durationMs = 700) {
            if (!this.camera)
                return;
        ...
        switchFloorByPisoName(pisoStr, edificio) {
        ...
        async onDestinationSelected(destinationName) {
            this.selectedDestination = destinationName;
            const result = await this.mapNavService.calculateRoute(destinationName, (locName, cuerpoId) => this.babylonSceneService.getMeshWorldPosition(locName, cuerpoId));
            if (result) {
                if (result.piso) {
        ...
        isFloorSelectionTrigger(meshName) {
        ...
        getBuildingATransitionMarkerState() {
        ...
        dispose3d() {
            if (this.engine) {
                ...
                if (this.scene && this.cameraBeforeRenderObserver) {
                    try {
                        this.scene.onBeforeRenderObservable.remove(this.cameraBeforeRenderObserver);
                    }
                    catch { }
        ...
        updateOrthoCamera() {
            if (!this.camera || !this.engine || this.camera.mode !== BABYLON.Camera.ORTHOGRAPHIC_CAMERA)
                return;
                ...
                canvas.addEventListener('click', (evt) => {
                    ...
                    if (pickResult?.hit) {
                        ...
                        if (isFirstFloorBodyDetailTrigger || isSecondFloorBodyDetailTrigger || isThirdFloorBodyDetailTrigger) {
                            ...
                            const isSuppressedLeftPanel = (this.currentFloor === this.firstFloorModel || this.currentFloor === this.buildingBFirstFloorModel)
                                && excludedFirstFloorDetailBodies.includes(normalizedMeshName);
        ...
        createGround() {
            ...
            const createWall = (name, width, height) => {
        ...
        openDetailPanel(title, contentHtml) {
            this.isDetailPanelOpen = true;
        ...
        updateBuildingBMarkerPosition(canvas) {
            // Placeholder para actualizar posición de marcadores en pantalla
        }
        formatBodyPanelTitle(meshName) {
    ...
    return Map3dContainerComponent = _classThis;
})();
export { Map3dContainerComponent };
